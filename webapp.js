$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    ...
    "columns": [
      ...
      { "data": "ICNO" },
      { "data": "PRNO",        "sClass": "integer" },
      { "data": "Address",    "sClass": "integer" },
      { "data": "Occupation",      "sClass": "integer" },
  ...
  $(document).on('click', '#add_company', function(e){
    ...
    $('#form_company #icno').val('');
    $('#form_company #prno').val('');
    $('#form_company #address').val('');
    $('#form_company #occupation').val('');
    $('#form_company #age').val('');
    $('#form_company #gender').val('');
    $('#form_company #contactNo').val('');
    $('#form_company #nationality').val('');
    $('#form_company #emergencyContact').val('');
    show_lightbox();
  });
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    request.done(function(output){
      if (output.result == 'success'){
        ...
        $('#form_company #icno').val(output.data[0].ICNO);
        $('#form_company #prno').val(output.data[0].PRNO);
        $('#form_company #address').val(output.data[0].Address);
        $('#form_company #occupation').val(output.data[0].Occupation);
        $('#form_company #age').val(output.data[0].Age);
        $('#form_company #gender').val(output.data[0].Gender);
        $('#form_company #contactNo').val(output.data[0].ContactNo);
        $('#form_company #nationality').val(output.data[0].Nationality);
        $('#form_company #emergencyContact').val(output.data[0].EmergencyContact);
        hide_loading_message();
        show_lightbox();
      } else {
        hide_loading_message();
